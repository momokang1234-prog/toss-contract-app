...
Identify where the comments are stored. Depending on the test session state, comments are typically captured as part of the `StateSnapshot` under `issues` or within a dedicated mock database / local JSON file (e.g., `server/ux-test-sessions/`).
...
- **Dispatch**: 
  - For simple, localized styling/prop changes: Apply the changes directly using `edit`.
  - For complex state logic or cross-file architectural changes: Delegate to the appropriate subagent (e.g., `task`, `toss-app-dev:toss-mini-app`, `designer`) using the `task` tool, passing the context of the comment and the target component.
