/**
 * Step1 기본정보 — Variant D: bold ProgressBar + "N/7 단계" 텍스트만 — 미니멀 Toss 스타일
 * design session: 20260618_050000
 */
import { useState } from 'react';
import { ProgressBar, TextField, Spacing, Paragraph } from '@toss/tds-mobile';
...
export default function FormVariantD() {
  ...
  const progress = (STEP_INDEX + 1) / STEPS.length;

  return (
    <div style={{ background: '#fff', minHeight: '100vh', maxWidth: 480, margin: '0 auto' }}>
      <CommentBoundary name="진행표시-미니멀">
        <div style={{ padding: '20px 24px 0' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 10 }}>
            ...
            <Paragraph typography="st6" color="grey-500">
              {STEP_INDEX + 1}/{STEPS.length} 단계
            </Paragraph>
          </div>
          <ProgressBar progress={progress} size="bold" />
        </div>
        <Spacing size={28} />
      </CommentBoundary>
