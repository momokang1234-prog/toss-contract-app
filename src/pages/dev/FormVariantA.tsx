/**
 * Step1 기본정보 — Variant A: dot stepper(타이틀 없음) + 우측 Badge로 현재 스텝명 강조
 * design session: 20260618_060000
 */
import { useState } from 'react';
import { ProgressStepper, ProgressStep, Badge, TextField, Spacing, Paragraph } from '@toss/tds-mobile';
...
export default function FormVariantA() {
  ...
  return (
    <div style={{ background: '#fff', minHeight: '100vh', maxWidth: 480, margin: '0 auto' }}>
      <CommentBoundary name="진행표시-dot+badge">
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingRight: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 2, paddingTop: 16 }}>
            <Badge size="medium" variant="fill" color="blue">
              {STEPS[STEP_INDEX]}
            </Badge>
            <Paragraph typography="st7" color="grey-500">{STEP_INDEX + 1}/{STEPS.length}단계</Paragraph>
          </div>
        </div>
        <Spacing size={20} />
      </CommentBoundary>
