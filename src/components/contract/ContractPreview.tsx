...
export function ContractPreview({ contract }: ContractPreviewProps) {
  ...
  return (
    <div>
      {/* Contract content rendered as styled divs */}
      <div style={{ border: '1px solid #E5E8EB', borderRadius: 12, overflow: 'hidden' }}>
        ...
        <div style={{ padding: '20px 16px', backgroundColor: '#F9FAFB', borderBottom: '1px solid #E5E8EB', textAlign: 'center' }}>
          <h3 style={{ fontSize: 18, fontWeight: 700 }}>근로계약서</h3>
        </div>
        ...
        <div style={{ padding: 16 }}>
          ...
          {contract.worker_signature_data && (
            <div style={{ marginTop: 16, padding: 12, backgroundColor: '#F9FAFB', borderRadius: 8 }}>
              <p style={{ fontSize: 13, fontWeight: 600, marginBottom: 8 }}>근로자 서명</p>
      ...
      <button
        ...
        style={{
          ...
          backgroundColor: '#F5F6F8', border: '1px solid #E5E8EB', borderRadius: 10,
          fontSize: 14, fontWeight: 500, cursor: downloading ? 'not-allowed' : 'pointer',
          color: '#333D4B',
        }}
      >
...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return <p style={{ fontSize: 15, fontWeight: 700, marginTop: 20, marginBottom: 8, color: '#191F28' }}>{children}</p>;
}

function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ display: 'flex', borderBottom: '1px solid #F2F4F6', padding: '8px 0' }}>
      <span style={{ width: 100, flexShrink: 0, fontSize: 13, color: '#6B7684', fontWeight: 500 }}>{label}</span>
      <span style={{ fontSize: 14, color: '#191F28' }}>{value}</span>
    </div>
  );
}
